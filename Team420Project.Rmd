...
library(kableExtra)
```
Loaded all of libraries required for methods and analysis
...
head(final_table)
```
Cleaned data to optimize quality of observations and analysis. First the data and results are read from their respective .csv files. The rows that do not have a state abbreviation are then removed.
The first for loop attaches a new column to the end of the predictors .csv file with the proportion of Hillary Votes. The DC variable is removed as it adds a new binary variable due to one-hot encoding and only has one respective row. The states are broke down into regions sinces reduces the dimensionality of the data. Without this modification, the full interaction model would have more coefficients than there exists data points (rows) in the dataframe. This is because of the one-hot encoding of our categorical variable (state). While this categorization does result in a loss of information, it does make future computations easier and allows for p-values to exist.
...
* Hand selected valuable interactions. Original model is too big for AIC or BIC
...
* Q-Q Plot: as the Q-Q plot nearly follows a straight line, this suggests the data does in fact come from a normal distribution.  
...
* We reject the null and choose the smaller model. This shows us that less interactions with more significant independent predictors will yield a higher r{2} value and more normality
