...
```{r}
initial_mod = lm(HSD410213 ~ RHI125214, data=pruned_data)
...
# using aic bic to remove unvaluable predictors
...
```{r}
# saw line did bptest to double check
...
```{r}
#compare full additive to smaller ones, keep smaller because high p value so we reject the null
...
# hand selected valuable interactions, used it bc original model is too big to bic/aic
...
```{r}
# compare smaller to bigger model
...
tot = 0
for (x in states) {
  if (nrow(final_table[final_table$state_abbreviation == x, ]) < 45) {
